...
from cslr_vsl.evaluation.slr_eval.wer_calculation import evaluate
...
def seq_eval(cfg, loader, model, device, mode, epoch, work_dir, recoder,
    ...
    for batch_idx, data in enumerate(tqdm(loader)):
        ...
        total_info += [file_name.split("|")[0] for file_name in data[-1]]  # File info (split based on delimiter)

...
dataset = 'VSL_V2'
prefix = '/home/martinvalentine/Desktop/CSLR-VSL/data/interim/256x256px'
dict_path = f'/home/martinvalentine/Desktop/CSLR-VSL/data/processed/{dataset}/gloss_dict.npy'
model_weights = '/home/martinvalentine/Desktop/CSLR-VSL/outputs/logs/V2/v2_aug/_best_model.pt'  # TODO: replace with model path
select_id = 15  # The video selected to show

# Load data and apply transformation
gloss_dict = np.load(dict_path, allow_pickle=True).item()
inputs_list = np.load(
    f"/home/martinvalentine/Desktop/CSLR-VSL/data/processed/{dataset}/dev_info.npy",
    allow_pickle=True
).item()
name = inputs_list[select_id]['fileid']

print(f'Generating CAM for {name}')
img_folder = os.path.join(prefix, dataset, inputs_list[select_id]['folder'])
...
video_length = torch.LongTensor([
    np.ceil(vid.size(1) / total_stride) * total_stride + 2 * left_pad
])
...
def cam_show_img(img, feature_map, grads, out_dir):  # img: ntchw, feature_map: ncthw, grads: ncthw
    ...
    for i in range(T):
        out_cam = cam[i]
        out_cam -= np.min(out_cam)
        out_cam /= (1e-7 + out_cam.max())
